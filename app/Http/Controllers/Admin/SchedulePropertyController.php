...
class SchedulePropertyController extends Controller
{
    public function index()
    {
        ...
        // dd($visiterInfo);
        return view('admin.schedule_properties.index', compact('visiterInfo'));
    }

    public function create()
    {
        return view('admin.schedule_properties.create');
    }
    ...
    public function update_field_manager(Request $request)
    {
        ...
        return redirect()->route('admin.schedule_properties.visit', ['property_id' => $propertyId]);
    }
    public function visit($scheduleVisitId)
    {
        ...
        // dd($visiterInfo->property->city);
        $cityName =  Cities::where('city_id', $visiterInfo->property->city)->first();
        $admin = Auth::guard('admin')->user();
        if ($admin) {
            if (is_null($visiterInfo->staff_id)) {
                $visiterInfo->staff_id = $admin->id;
                $visiterInfo->save();
            }
        } else {
            return response()->json(['message' => 'Unauthorized admin user.'], 401);
        }
        $fieldManager = FieldManager::all();
        // $user = User::all();
        return view('admin.schedule_properties.show', compact('visiterInfo', 'fieldManager', 'cityName'));
    }
    ...
    public function view($scheduleVisitId)
    {
        ...
        // dd($visiterInfo);
        // $fieldManager = FieldManager::all();
        return view('admin.schedule_properties.view', compact('visiterInfo'));
    }
    public function store(Request $request)
    {
        ...
        return redirect()->route('admin.schedule_properties.index');
    }
    ...
    public function edit($id)
    {
        ...
        // dd($visiterInfo);
        return view('admin.schedule_properties.edit', compact('visiterInfo'));
    }
    ...
    {
        try {
            $scheduleVisit = ScheduleVisit::where('id',$id)->first();

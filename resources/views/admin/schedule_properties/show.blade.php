        ...
        <div class="card">
            ...
            <div class="card-body mt-3">
                ...
                <small class="text-muted">
                    Posted By <strong>{{ $visiterInfo->property->owner->name }}</strong>
                    on {{ $visiterInfo->property->created_at->toFormattedDateString() }}
                </small>
            </div>
        ...
        <div class="card">
            ...
            <div class="card-body mt-3">
                <ul class="list-group">
                    <li class="list-group-item">
                        <strong>Name:</strong>
                        <span class="right">{{$visiterInfo->property->owner->name}}</span>
                    </li>
                    <li class="list-group-item">
                        <strong>Email:</strong>
                        <span class="right">{{$visiterInfo->property->owner->email}}</span>
                    </li>
                    <li class="list-group-item">
                        <strong>Mobile:</strong>
                        <span class="right">{{$visiterInfo->property->owner->mobile_no}}</span>
                    </li>
                    ...
                    <li class="list-group-item">
                        ...
                        <p>
                            ...
                            <span class="right">{{ Carbon\Carbon::parse($visiterInfo->conform_timing->timing)->format('l, F j, Y \a\t g:i A') }}</span>
                        </p>
                        @else
                        <p> <strong>Timing:</strong>
                            <a href="{{route('admin.owner.send-time-confirmation',$visiterInfo->property->unique_id)}}" class="btn btn-success waves-effect">
...
<div class="row clearfix">
                    ...
                    <div class="form-group form-float">
                        ...
                        <div class="form-line">
                            <select id="owner-select" name="owner" class="form-control  show-tick">
                                <option value="{{ $visiterInfo->property->owner->id }}" selected>{{ $visiterInfo->property->owner->name }}</option>
                            </select>
                        </div>
                        @error('owner')
                        <span class="text-danger">{{ $message }}</span>
                    ...
                    <div class="form-group form-float mb-2">
                        ...
                        <div class="form-line">
                            <input type="text" class="form-control datetimepicker" name="timing"
                                value="{{ old('timing', \Carbon\Carbon::parse($visiterInfo->conform_timing->timing)->format('Y-m-d h:i A')) }}"
                                readonly>
                        </div>
                        @else
    ...
    <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
        <div class="card">
            ...
            <div class="card-body mt-3">
                <form action="{{ route('admin.schedule_properties.update_field_manager') }}" method="POST">
                    ...
                    <ul class="list-group">
                        ...
                        <li class="list-group-item">
                            ...
                            <p><strong>Timing:</strong>
                                <span id="timing_display" class="right">{{ \Carbon\Carbon::parse($visiterInfo->conform_timing->timing)->format('l, F j, Y \a\t g:i A') }}</span>
                            </p>
                            <input type="hidden" id="conform_timing" name="conform_timing" value="{{ $visiterInfo->conform_timing->id }}">
                            @else
                            <p><strong>Timing:</strong>
                                <a href="{{ route('admin.field_manager.send-time-confirmation', [$visiterInfo->conform_timing->field_manager_id, $visiterInfo->property->unique_id]) }}" class="btn btn-success waves-effect">
                                    <svg class="MuiSvgIcon-root MuiSvgIcon-fontSizeMedium css-9gwpq2" focusable="false" aria-hidden="true" viewBox="0 0 24 24" data-testid="WhatsAppIcon" style="user-select: none;width: 24px;display: block;flex-shrink: 0;fill: currentcolor;font-size: 210px;color: rgb(255, 255, 255);">
                                        <path d="M16.75 13.96c.25.13.41.2.46.3.06.11.04.61-.21 1.18-.2.56-1.24 1.1-1.7 1.12-.46.02-.47.36-2.96-.73-2.49-1.09-3.99-3.75-4.11-3.92-.12-.17-.96-1.38-.92-2.61.05-1.22.69-1.8.95-2.04.24-.26.51-.29.68-.26h.47c.15 0 .36-.06.55.45l.69 1.87c.06.13.1.28.01.44l-.27.41-.39.42c-.12.12-.26.25-.12.5.12.26.62 1.09 1.32 1.78.91.88 1.71 1.17 1.95 1.3.24.14.39.12.54-.04l.81-.94c.19-.25.35-.19.58-.11l1.67.88M12 2a10 10 0 0 1 10 10 10 10 0 0 1-10 10c-1.97 0-3.8-.57-5.35-1.55L2 22l1.55-4.65A9.969 9.969 0 0 1 2 12 10 10 0 0 1 12 2m0 2a8 8 0 0 0-8 8c0 1.72.54 3.31 1.46 4.61L4.5 19.5l2.89-.96A7.95 7.95 0 0 0 12 20a8 8 0 0 0 8-8 8 8 0 0 0-8-8z"></path>
                                    </svg>
                                </a>
...
<script>
    $(document).ready(function() {
        ...
        $('#field_manager').change(function() {
            ...
            $.ajax({
                url: '{{ route("admin.schedule_properties.get-field-manager") }}', // Adjust the route as needed
                method: 'GET',
                data: {
                    id: fieldManagerId,

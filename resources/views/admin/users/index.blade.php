                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    @if($user->email)
                                    @php
                                    $scheduleCount = \App\Models\ScheduleProperties::where('email', $user->email)->count();
                                    @endphp
                                    {{ $scheduleCount }}
                                    @endif
                                ...
                                <td>
                                    <div class="text-center d-flex justify-content-center align-items-center">
                                        <a href="{{ route('admin.user.edit', $user->id) }}" class="btn btn-info btn-sm mx-1">
                                            <i class="bi bi-pencil"></i> <!-- Bootstrap Icon for edit -->
                                        </a>
                                        <button type="button" class="btn btn-danger btn-sm mx-1" onclick="deleteUser({{ $user->id }})">
                                            <i class="bi bi-trash"></i><!-- Bootstrap Icon for delete -->
                                        </button>
                                        <form action="{{ route('admin.user.destroy', $user->id) }}" method="POST" id="del-user-{{$user->id}}" style="display:none;">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                        <a href="{{ route('admin.user.verify', $user->id) }}" class="btn btn-info btn-sm mx-1" data-toggle="tooltip" data-placement="top" title="Verify User">

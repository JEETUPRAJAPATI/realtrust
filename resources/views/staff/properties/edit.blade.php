...
<div class="row clearfix">
    <form action="{{ route('staff.properties.update', $property->slug) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="col-sm-12 col-xs-12" style="display:flex;gap:10px;">
            <div class="col-lg-8 col-md-4 col-sm-12 col-xs-12">
                    ...
                    <div class="card-body mt-3">
                        ...
                        <div class="form-group form-float mb-2">
                            <div class="form-line {{ $errors->has('purpose') ? 'focused error' : '' }}">
                                ...
                                <select name="purpose" id="purposeSelect" class="form-control show-tick">
                                    <option value="">-- Please select --</option>
                                        <option value="sell" {{ strtolower(old('purpose', $property->purpose)) == 'sell' ? 'selected' : '' }}>Sell</option>
                                        <option value="rent" {{ strtolower(old('purpose', $property->purpose)) == 'rent' ? 'selected' : '' }}>Rent</option>
                                        <option value="upcoming_projects" {{ strtolower(old('purpose', $property->purpose)) == 'upcoming_projects' ? 'selected' : '' }}>Upcoming Projects</option>
                                </select>
                            </div>
                            @error('purpose')
                            <span class="text-danger">{{ $message }}</span>
                        ...
                        <div class="form-group form-float mb-2">
                            <div class="form-line">
                                ...
                                <input type="text" placeholder="Enter Block Number" class="form-control" name="block_no" value="{{ old('block_no', $property->block_no) }}">
                            </div>
                            @error('block_no')
                            <span class="text-danger">{{ $message }}</span>
                            @enderror
                        ...
                        <div class="form-group form-float mb-2">
                            <div class="form-line">
                                ...
                                <input type="text" placeholder="Enter Flat Number" class="form-control" name="flat_no" value="{{ old('flat_no', $property->flat_no) }}">
                            </div>
                            @error('flat_no')
                            <span class="text-danger">{{ $message }}</span>
                            @enderror
                ...
                <div class="card">
                    ...
                    <div class="card-body mt-3">
                        <div class="form-group form-float mb-2 {{ $errors->has('price') ? 'has-error' : '' }}" id="priceSection" style="display: {{ $errors->has('price') ? 'block' : 'none' }};">
                            <div class="form-line">
                    ...
                    <div class="card">
                        ...
                        <div class="card-body mt-3">
                            <div class="gallery-box" id="gallerybox">
                                ...
                                @endforeach
                                <input type="file" name="galleryimage[]" id="input-id" class="file12" multiple>
                            </div>
                ...
                <div class="card">
                    ...
                    <div class="form-group form-float mb-2">
                            <div class="form-line {{ $errors->has('tenant_type') ? 'focused error' : '' }}">
                                ...
                                @php
                                    $selectedTenantType = strtolower(old('tenant_type', $property->tenant_type));
                                @endphp
                                
                                <select name="tenant_type" class="form-control show-tick">
                                    <option value="">-- Please select --</option>
                                    <option value="family" {{ $selectedTenantType == 'family' ? 'selected' : '' }}>Family</option>
                                    <option value="bachelorBoy" {{ $selectedTenantType == 'bachelorboy' ? 'selected' : '' }}>BachelorBoy</option>
                                    <option value="bachelorGirls" {{ $selectedTenantType == 'bachelorgirls' ? 'selected' : '' }}>BachelorGirls</option>
                                    <option value="anyone" {{ $selectedTenantType == 'anyone' ? 'selected' : '' }}>AnyOne</option>
                                </select>
                ...
                <div id="floorPlanSection" style="display: none;">
                    <div class="card">
                        ...
                        <div class="card-body mt-3 text-center">
                            <img id="floorPlanImagePreview"
                                 src="{{ $property->floor_plan && Storage::disk('public')->exists('property/' . $property->owner_id . '/' . $property->unique_id . '/' . $property->floor_plan) 
                                        ? Storage::url('property/' . $property->owner_id . '/' . $property->unique_id . '/' . $property->floor_plan)
                                        : 'https://via.placeholder.com/200x150?text=No+Floor+Plan' }}"
                                 alt="Floor Plan"
                                 class="img-fluid img-thumbnail rounded mb-2"
                                 style="max-width: 200px; height: auto;">
                
                            <input type="file" name="floor_plan" id="floorPlanImageInput" class="d-none" accept="image/*">
                
                            <button type="button" class="btn btn-outline-primary btn-sm" onclick="document.getElementById('floorPlanImageInput').click();">
                                <i class="bi bi-pencil-square"></i> Edit Floor Plan
                            </button>
                         </div>
                    </div>
                </div>
    ...
    <div class="card-body mt-3 text-center">
        <img id="featuredImagePreview"
             src="{{ $property->image && Storage::disk('public')->exists('property/' . $property->owner_id . '/' . $property->unique_id . '/' . $property->image)
                    ? Storage::url('property/' . $property->owner_id . '/' . $property->unique_id . '/' . $property->image)
                    : 'https://via.placeholder.com/200x150?text=No+Image' }}"
             alt="Featured Image"
             class="img-fluid img-thumbnail rounded mb-2"
             style="max-width: 200px; height: auto;">

        <input type="file" name="image" id="featuredImageInput" class="d-none" accept="image/*">

        <button type="button" class="btn btn-outline-primary btn-sm" onclick="document.getElementById('featuredImageInput').click();">
            <i class="bi bi-pencil-square"></i> Edit Featured Image
        </button>
...
<script>
  ...
  function previewImage(inputId, previewId) {
    ...
    previewImage('featuredImageInput', 'featuredImagePreview');
    previewImage('floorPlanImageInput', 'floorPlanImagePreview');
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('.gallery-image-edit button').on('click', function(e) {
            ...
            $.ajax({
                url: "{{ route('staff.gallery-delete') }}", // Laravel route for deletion
                type: "POST",
                data: {
                    id: id,
                    image: image,
